...
三个 Next.js 后端端口为 `127.0.0.1:13101`、`:13102`、`:13103`。Nginx 对公网提供 HTTPS 端口 `3101`、`3102`、`3103`，分别对应三个区域。
...
Nginx 对公网监听 `3101`、`3102`、`3103`（HTTPS，供 IP 直连使用），并分别代理到本机的 `13101`、`13102`、`13103`。不要把 Resin `2260` 暴露给公网。

## 客户端入口

Cloudflare Tunnel 不作为生产入口。BBZQ 使用服务器 IP 和 HTTPS：

| BBZQ 区域 | 地址 |
| --- | --- |
| 香港 | `https://47.98.174.251:3101` |
| 台湾 | `https://47.98.174.251:3102` |
| 国际 | `https://47.98.174.251:3103` |
